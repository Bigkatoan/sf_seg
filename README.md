...
│
├── train_sf_seg.py             # Wrapper (imports from src.training)
├── prepare_ade20k.py           # Wrapper (imports from src.dataloaders.ade20k)
├── prepare_data.py             # Wrapper (imports from src.dataloaders.utils)
├── visualize_attention.py      # Wrapper (imports from src.visualization.attention)
├── draw_arch.py                # Wrapper (imports from src.visualization.architecture)
├── evaluate_lvis.py            # Wrapper (imports from src.visualization.evaluation)
├── benchmark.py                # Wrapper (imports from src.utils.benchmark)
│
...
**Note:** All root-level scripts (`train_sf_seg.py`, `prepare_ade20k.py`, etc.) are lightweight wrappers that import from the corresponding `src/` modules. Users can either call the wrapper or import from `src/` directly in custom scripts.
